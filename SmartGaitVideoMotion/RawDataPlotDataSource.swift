...
final class RawDataPlotDataSource: NSObject, VideoMotionDataPlotDataSource {
  typealias Data = RawMotionData

  var iOSMotionData: [RawMotionData]
  var watchOSMotionData: [RawMotionData]

  var iOSPlotMotionData: [RawMotionData]
  var watchOSPlotMotionData: [RawMotionData]
  ...
  init(iOSMotionData: [RawMotionData], watchOSMotionData: [RawMotionData], iOSPlotMotionData: [RawMotionData] = [], watchOSPlotMotionData: [RawMotionData] = []) {
...
extension RawDataPlotDataSource {
...
extension RawDataPlotDataSource {
  ...
  func symbol(for plot: CPTScatterPlot, record idx: UInt) -> CPTPlotSymbol? {
    ...
    switch graphIdentifier {
    case .iOSGravityX, .iOSGravityY, .iOSGravityZ:
      if let zLabel = iOSMotionData[Int(idx)].zLabel, zLabel != "?" {
    ...
    case .watchOSGravityX, .watchOSGravityY, .watchOSGravityZ:
      if let zLabel = watchOSMotionData[Int(idx)].zLabel, zLabel != "?" {
        if zLabel.lowercased() == "balanced" {
          symbol.fill = CPTFill(color: .blue())
        } else if zLabel.lowercased() == "imbalanced" {
